# Flash Sale System - Ultra-Comprehensive Architecture Design (V3)
...
│     the master can handle all traffic:
│     - Master handles 200k reads + 25k invalidations = 225k RPS total
...
│     - Master capacity sufficient for single SKU flash sale (r6g.4xlarge: 400k+ ops/sec)
│
├─ Option 5: Memcached
...
- Shard A (stock:{sku_id}): Handles all stock availability traffic
- Shard A configuration:
  ├─ Master: Handles ALL operations (200k reads + 25k invalidations = 225k RPS)
  │   ├─ Cannot use read-from-replica due to replication lag (10-100ms)
  │   ├─ Even 10ms lag = 2,000 users see stale data (unacceptable UX)
